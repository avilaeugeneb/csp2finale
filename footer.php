	


<!--Footer-->
<footer class="page-footer font-small brown darken-4 pt-4 mt-4 fixed-bottom">

	<!--Footer Links-->
	<div class="container-fluid text-center text-md-left">
		<div class="row">

			<!--First column-->
			<div class="col-md-4">
				<h5 class="text-uppercase">Contact Us</h5>
				<p>Phone: (02)861-5555</p>
				<p>Email: admin@purered.com.ph</p>
				<p>Address:</p>
				<p> #121 Mahal Ka Nya St.</p>
				<p>Brgy.Kinalimutan, Pinaasa City</p>
				<p>Republic of Hugotero't Hugotera</p>
			</div>
			<!--/.First column-->

			<!--Second column-->
			<div class="col-md-4">
				<h4 class="text-uppercase">PURE Group of Companies</h4>
				<h6 class="text-uppercase">Pure.Food</h6>
				<h6 class="text-uppercase">Pure.Tools</h6>
				<h6 class="text-uppercase">Pure.Style</h6>
				<h6 class="text-uppercase">Pure.White</h6>
			</div>
			<!--/.Second column-->

			<!--Third column-->
			<div class="col-md-4">
				<h4 class="text-uppercase">TERMS OF USE</h4>
				<h4 class="text-uppercase">PRIVACY POLICY</h4>
				<h4 class="text-uppercase">SITEMAP</h4>
				<h4 class="text-uppercase">COPYRIGHT POLICY</h4>
				<h4 class="text-uppercase">CREDITS</h4>
			</div>
			<!--/.Third column-->
		</div>
	</div>
	<!--/.Footer Links-->

	<!--Copyright-->
	<div class="footer-copyright py-3 text-center">
		<a href="#"> PureRed : Ang Online Pamilihan ng Bayan </a>
	</div>
	<!--/.Copyright-->

</footer>
<!--/.Footer-->

<!-- SCRIPTS -->
<!-- JQuery -->
<script src="assets/js/jquery-3.2.1.min.js"></script>
<!-- Bootstrap tooltips -->
<script src="assets/js/popper.min.js"></script>
<!-- Bootstrap core JavaScript -->
<script src="assets/js/bootstrap.min.js"></script>
<!-- MDB core JavaScript -->
<script src="assets/js/mdb.min.js"></script>
</body>

</html>
